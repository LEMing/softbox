...
export class PostProcessingPipeline {
  ...
  private build(modules: PostModules, scene: THREE.Scene, camera: THREE.Camera): Composer {
    ...
    if (this.config.bloom && modules.UnrealBloomPass) {
      ...
      // (headlights, hot speculars) seed the glow, so it reads as a studio
      // touch rather than a haze over the whole brightly-lit scene. Params:
      // (resolution, strength, radius, threshold).
      composer.addPass(
        new modules.UnrealBloomPass(new THREE.Vector2(bufferSize.x, bufferSize.y), 0.18, 0.25, 1.0) as object
      );
    }
